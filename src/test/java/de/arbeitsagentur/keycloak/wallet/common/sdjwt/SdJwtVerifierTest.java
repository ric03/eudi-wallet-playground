...
class SdJwtVerifierTest {
    ...
    void setUp() throws Exception {
        trustListService = new TrustListService(objectMapper);
        trustListService.load();
    }
    ...
    void verifiesSdJwtWithKeyBinding() throws Exception {
        MockIssuerProperties props = new MockIssuerProperties(
                true,
                java.nio.file.Path.of("config/mock-issuer-keys.json"),
                Duration.ofMinutes(5),
                "https://issuer.example/mock",
                java.nio.file.Path.of("config/mock-issuer-configurations.json"),
                List.of()
        );
        ...
        SdJwtCredentialBuilder builder = new SdJwtCredentialBuilder(objectMapper, keyService, props);
        ...
        SdJwtVerifier verifier = new SdJwtVerifier(objectMapper, trustListService);
        Map<String, Object> claims = verifier.verify(sdJwt, "trust-list-mock", "aud-123", "nonce-123", keyBindingJwt, new VerificationSteps());
